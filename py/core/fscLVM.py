...
class CSparseFA(AExpressionModule):
    ...
    def getName(self,base_name='fscLVM'):
        ...
        name = "%s %s, %s" % (base_name,self.nHidden,self.nKnown)
        return name
    ...
    def updateEpsDrop(self):
        ...
        t3 = SP.zeros((self._D,))
        mRangeUse = SP.where(self.doUpdate>=0)[0]
    ...
    def update(self):
        ...
        if self.noise=='drop' or self.noise=='poisson':
            ...
            self.meanX[self.isExpressed==0] = (Xi - (1./(1.+SP.exp(-Xi)))/self.Eps.E1)[self.isExpressed==0]
