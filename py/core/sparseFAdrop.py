...
#        CNodeS.__init__(self,net,prec=1)
        ...
        self.E1 = random.standard_normal(size=[net._N,net.components])
        ...
        for m in mRange:
            if self.doUpdate[m]==1:
               ...
               #  for m in mRange:
                    ...
                    self.doUpdate[m]=0
                    print 'Factor:', m, 'iter:', self.iterationCount


        self.updateEps()
        #update xi folowing Seeger et al 2012, AISTATS
        Xi = SP.dot(self.S.E1,(self.W.C[:, :,0]*self.W.E1).transpose())
        #Xi = SP.dot(self.S.E1[:,self.doUpdate==1],(self.W.C[:, self.doUpdate==1,0]*self.W.E1[:,self.doUpdate==1]).transpose())
...
#            pdb.set_trace()
    ...
    def init(self,init_data,Pi=None, init_factors=None):
        ...
        self.doUpdate = SP.ones((Pi.shape[1],))
