...
#        #experiments(EP)
    ...
    def update(self):
        ...
        self.meanX[self.isExpressed==1] = (Xi - self.fprime(Xi, self.Z.E1)/SP.repeat(self.kappa[:,SP.newaxis],self._N,1).T)[self.isExpressed==1]
...
#                self.W.E1[~Ion[:,k],k]*=self.sigmaOff
        #Xi = SP.dot(self.S.E1,(self.W.C[:, :,0]*self.W.E1).transpose() )
        self.meanX = SP.log2(self.Z.E1.astype('float64')+1)#self.Z.E1.copy()#Xi - self.fprime(Xi, self.Z.E1)/self.kappa
        self.initS = self.S.E1.copy()
