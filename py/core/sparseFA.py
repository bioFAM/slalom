...
class CNodeWsparseEP(CNodeW):
    ...
    def __init__(self,net,prec=1):
        ...
        self.Pi[:,:,1] =net.Pi
        self.Pi[:,:,0] = 1-net.Pi
        self.prec = S.array([net.sigmaOff**(-2),1])
        #log prior
        self.lpC1 = log(net.Pi)
        self.lpC0 = log(1-net.Pi)
        self.C    = self.Pi.copy()
...
#        self.nodes = {'S':CNodeS(self),'W':W_node,'Eps':CNodeEpsFix(self,self.priors['Eps']['priors'])}
        self.nodes = {'S':CNodeS(self),'W':W_node,'Eps':CNodeEps(self,self.priors['Eps']['priors'])}
        ...
        if 1:
            ...
            if Ion is not None:
                for d in xrange(self._D):
                    pprior = diag(1.0*Ion[d,:]+(~Ion[d,:])*self.sigmaOff**(-2))
