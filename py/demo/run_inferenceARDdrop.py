...
if __name__ == '__main__':
    ...
    if 'cluster' in sys.argv:
        ...
        anno = 'MSigDB'
        nHidden = 3
        idx_known = [0]#SP.arange(8,18)#SP.hstack([SP.arange(1,4),SP.arange(8,18)])
        nIterations = 10
        idxCol=[]
        doFast=True
    ...
    if doFast==True:
        idx_genes  = SP.logical_and(SP.sum(pi>.5,1)>0, Y.mean(0)>-10.)#SP.any(pi>.5,1)
        Y = Y[:,idx_genes]
        pi = pi[idx_genes,:]
    ...
    init={'init_data':sparseFA.CGauss(Y),'Pi':pi,'init_factors':init_factors}
    FA = sparseFA.CSparseFA(components=K,sigmaOff=sigmaOff,sigmaOn=SP.ones(pi.shape[1])*1.0,sparsity=sparsity,nIterations=nIterations,permutation_move=permutation_move,priors=priors,initType='random')            
    FA.shuffle=True
