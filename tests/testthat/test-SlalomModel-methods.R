...
test_that("SlalomModel works with covariates", {
    ...
    ## access Python results if needed
    library(rhdf5)
    h5file <- file.path("./inst", "extdata", "outSim", "N_20_20it_cov.h5py")
    h5ls(h5file)
    ...
    ## test R and Py results with exactly the same initialisation
    m <- newSlalomModel(sce, genesets[1:23], n_hidden = 2, min_genes = 1,
    ...
    plot(t(h5read(h5file, "W")[,,2])[,1], m$W_E1[,1])
    abline(0, 1, col = "firebrick")
    plot(t(h5read(h5file, "Pi_inferred")[,,2])[,1], m$W_gamma0[,1])
    abline(0, 1, col = "firebrick")
    plot(t(h5read(h5file, "X")[,,1])[,1], m$X_E1[,1])
    abline(0, 1, col = "firebrick")
