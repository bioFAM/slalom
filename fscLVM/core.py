...
class CSparseFA(AExpressionModule):
    ...
    def getAnnotations(self, annotated=True,unannotated=True,unannotated_sparse=True):
        """Get annotations.
        ...
        i_use = list()
        if unannotated_sparse==True:
            i_use.extend(self.iLatentSparse)
        if unannotated==True:
            i_use.extend(self.iLatent)
        if annotated==True:
            i_use.extend(SP.setxor1d(SP.hstack([self.iLatentSparse, self.iLatent]), SP.arange(len(self.terms))))
        i_use = SP.array(i_use)

        return (self.Pi[:,i_use]>.5)
