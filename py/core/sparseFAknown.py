    ...
    def iterate(self, nIterations=None, forceIterations=None, tolerance=None, minIterations=10):
        ...
        for iter in range(nIterations):
            ...
            #    self.updateNode(node)
                #pdb.set_trace()
            self.update()
            if self.permutation_move==True and SP.mod(iter,50)==0:
                #greedily select factors
#                print "tic"
#                C = self.W.C/SP.dstack([self.Pi, 1-self.Pi])
#                #normalise
...
##                M = SP.zeros([self.components,self.components])
##                for k in xrange(self.components):
##                    for l in xrange(self.components):
##                        M[k,l] = self.logProbkk(k,l)
##                        
##                print "logProbkk done"               
##                print M
                M = metrics.pairwise.pairwise_distances(self.S.E1.T,self.initS.T, metric='correlation')
#                #greedily select factors
                K = SP.arange(self.components)

                F = SP.arange(self.components)

                Ipi = SP.zeros(self.components,dtype='int')

                for k in K:
                    #get beset one
                    Ibest  = F[M[k,F].argmin()]
                    Ipi[k] = Ibest
                    #remove from list
                    F  = SP.setdiff1d(F,[Ibest])
                #print SP.sum(self.W.Pi[:,:,0]>.5,0)
                #print SP.sum(self.W.C[:,:,0]>.5,0)
                #print SP.sum(self.W.Pi[:,:,0]>.5,0)
                print self.Alpha.E1
                #pdb.set_trace()
                #keep track of the changes also
                print Ipi
                #self.W.Ilabel = self.W.Ilabel[Ipi]
                #update the prior Pi
                #self.Pi = self.Pi[:,Ipi]
                #self.W.Pi = self.W.Pi[:,Ipi]
                #and the precalculated log versions:
#                self.W.lpC1 = self.W.lpC1[:,Ipi]
#                self.W.lpC0 = self.W.lpC0[:,Ipi]
                #print "toc"
            ...
            error = (Zd**2).mean()
            self.calcBound()
            if (SP.mod(iter,100)==0 or iter<10):
                print "reconstruction error: %f lower bound: %f" % (error,  self._bound) 
